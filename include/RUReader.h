...
private:
  ...
  struct ChannelState {
    ...
    uint64_t wraps         = 0;   // wraps of the raw counter detected in software
    uint64_t rollOverFlags = 0;   // roll-over flags reported by the board
    uint64_t backwardJumps = 0;   // decreasing time stamps that are not a wrap
    uint64_t events        = 0;
